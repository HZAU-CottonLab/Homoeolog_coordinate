    ...
    with open(ofname, 'w') as out_fd:
        if opt.screen:
            ...
            for chrom in set(ELEMS['chrom']):
                tmpout=transform_by_chrom(EPO, ELEMS[ELEMS['chrom'] == chrom], TREE, chrom, opt, out_fd)
                out+=tmpout
    return out
    log.info("DONE!")
...
def index_chainFile(chainFile,opt):
    '''
    index the chain file
    args:
        chainFile: path of chain file
    return:
        EPO:  chain Object
        TREE: index of chain
    '''
    ...
    EPO = {ch[0].id: ch for ch in loadChains(chainFile)}
...
def getTargetLocation(requestBed,EPO,TREE,opt):
    '''
    get target location by only one request interval
    args:
      requestBed: ['Chr1',100,200,'intervalId'] @tuble
      chainFile: chainFile  @str
    return:
        array: [bed interval] @list(tuble)
    '''
    ...
    # parser.add_argument("-k", '--keep_split', default=True, action='store_true',)
    ...
    element_type = np.dtype([('chrom', np.str_, 30),
                         ('start', np.int64), ('end', np.int64), ('id', np.str_, 100)])                      
    inputBed = np.array(
        [requestBed], dtype=element_type)
    #########transform elements
    out=transform_file(inputBed, opt.output, EPO, TREE, opt,)        
    return out
...
if __name__ == "__main__":
    ...
    if len(opt.input) > 1:
        ...
        out=transform_file(loadFeatures(opt.input[0], opt), opt.output, EPO, TREE, opt,)
        print(out)
